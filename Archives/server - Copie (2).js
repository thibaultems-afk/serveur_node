...
app.post('/api/submit-case', upload.array('documents'), async (req, res) => {
  try {
    ...
    const {
      ...
      postalCode,
      country,
      typeDossier // typeId Kleos
    } = req.body;

    // --- Préparer mainAddress ---
    const mainAddress = {
      id: 0,
      street,
      houseNumber: '',
    ...
    // --- a) Créer contact ---
    const reference = `${nom}_${prenom}_${Date.now()}`;
    const contactPayload = {
      ...
      phoneNumber: '',
      readOnly: true,
      type: contactType === 'N' ? 'N' : 'L',
      reference,
      addresses: [mainAddress],
      mainAddress
    };

    console.log("=== Contact payload envoyé à Kleos ===");
    ...
    console.log("=== Contact créé avec identityId =", identityID);

    // --- b) Créer le dossier ---
    const now = new Date().toISOString();
    const caseReference = `CASE_${nom}_${prenom}_${Date.now()}`;
    const casePayload = {
      id: 0,
      title: `${nom} ${prenom}`,
      name: `${nom} ${prenom}`,
      reference: caseReference,
      typeId: parseInt(typeDossier, 10),
      creationDate: now,
      parties: [{ identityId: identityID }]
    };

    console.log("=== Case payload envoyé à Kleos ===");
    ...
    const caseID = caseResp.data.result;
    console.log("=== Dossier créé avec caseID =", caseID);
